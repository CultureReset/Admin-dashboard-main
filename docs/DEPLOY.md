...
psql "$GCR_DATABASE_URL" -f sql/industry_attributes.sql
```
...
| `industry_attributes.sql` | `entity_attributes` — the structured listing data guests search on, plus its four indexes |
